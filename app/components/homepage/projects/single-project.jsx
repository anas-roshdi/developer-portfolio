...
const SingleProject = ({ project }) => {
  ...
  // Handle both string paths and imported static objects correctly
  const imageSrc = typeof image === 'string' ? image : (image?.src || placeholder);
  ...
  return (
    <div className='group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] bg-[linear-gradient(90deg,#281e57_0%,#201435_100%)] shadow-2xl rounded-lg border border-[#1a1443]'>
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        <h2 className='text-[#EFF3F4] not-italic font-semibold text-[1.525rem] leading-[110%] text-center capitalize mb-2'>
          {name}
        </h2>
        <div className="p-6">
          <Image
        ...
        <div className="flex items-center justify-between w-full z-10">
          ...
          {/* Fix the Syntax Error on Link tag */}
          {code && (
            <Link
              href={code}
              target='_blank'
